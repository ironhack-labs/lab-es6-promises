...
async function makeBroccoli() {
  try {
    const value0 = await obtainInstruction("broccoli", 0);
    document.querySelector("#broccoli").innerHTML += `<li>${value0}</li>`;

    const value1 = await obtainInstruction("broccoli", 1);
    document.querySelector("#broccoli").innerHTML += `<li>${value1}</li>`;

    const value2 = await obtainInstruction("broccoli", 2);
    document.querySelector("#broccoli").innerHTML += `<li>${value2}</li>`;

    const value3 = await obtainInstruction("broccoli", 3);
    document.querySelector("#broccoli").innerHTML += `<li>${value3}</li>`;

    const value4 = await obtainInstruction("broccoli", 4);
    document.querySelector("#broccoli").innerHTML += `<li>${value4}</li>`;

    const value5 = await obtainInstruction("broccoli", 5);
    document.querySelector("#broccoli").innerHTML += `<li>${value5}</li>`;

    const value6 = await obtainInstruction("broccoli", 6);
...
const makeBrusselsSprouts = async () => {
  try {
    const promises = [];
    brusselsSprouts.forEach((step, index) => {
      promises.push(obtainInstruction("brusselsSprouts", index));
    });
