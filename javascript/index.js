...
for(let i = 0; i < 8; i++) {
  myPromiseArray.push(obtainInstruction('brusselSprouts', i));
}

Promise.all(myPromiseArray) 
.then(myResolvedArray) => {
  myResolvedArray.forEach(element => {
    document.querySelector('#brusselsSrpouts').innerHTML += `<li>`
  })
}