...
function makeBrusselSprouts() {
  Promise.all([
    ...
    .then((steps) => { // steps (can be any word) refers to the array of strings that we declare above ^
      steps.forEach((step) => { // for each element insidi array, we print the string of instruction (this is a loop)
        document.querySelector('#brusselsSprouts').innerHTML += `<li>${step}</li>`;
        document.querySelector('#brusselsSproutsImg').removeAttribute('hidden');
      });
      document.querySelector('#brusselsSprouts').innerHTML += `<li>Done Brussels and dat</li>`;
    })
    .catch((error) => {
      console.log(error);
    });
