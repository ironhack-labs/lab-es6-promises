...
const arrayOfSteps = [s0, s1, s2, s3, s4, s5, s6, s7];


Promise.all(arrayOfSteps)
  .then((values) => values.forEach((step) => document.querySelector("#brusselsSprouts").innerHTML += `<li>${step}</li>`))
  .then( () => document.querySelector("#brusselsSprouts").innerHTML += `<li>Brussels sprouts are ready!</li>`)
  .then( () => document.querySelector("#brusselsSproutsImg").hidden = false)
