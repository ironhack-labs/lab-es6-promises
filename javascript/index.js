    ...
    getInstruction("mashedPotatoes", 2, (step2) => {
      ...
      getInstruction("mashedPotatoes", 3, (step3) => {
        ...
        getInstruction("mashedPotatoes", 4, (step4) => {
          ...
          mashedPotatoesImg.style.display = "flex";
        });
      });
    });
  });
...
async function makeBroccoli() {
  try {
    ...
    const step2 = await obtainInstruction("broccoli", 0);
    document.querySelector("#broccoli").innerHTML += `<li>${step2}</li>`;
    const step3 = await obtainInstruction("broccoli", 0);
    document.querySelector("#broccoli").innerHTML += `<li>${step3}</li>`;
    const step4 = await obtainInstruction("broccoli", 0);
    document.querySelector("#broccoli").innerHTML += `<li>${step4}</li>`;

    document.querySelector(
      "#broccoli"
    ).innerHTML += `<li>${"Broccoli is ready!."}</li>`;
    broccoliImg.style.display = "flex";
  } catch (error) {
    console.log(error);
  }
}
...
Promise.all([
  ...
  .then(([step0, step1, step2, step3, step4, step5, step6, step7]) => {
    ...
    brusselsSproutsImg.style.display = "flex";
  })
  .catch((error) => {
    console.log(error);
  });
