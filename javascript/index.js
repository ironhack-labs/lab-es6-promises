...
// Bonus 2 - Promise all
const brusselsSprout = document.querySelector("#brusselsSprouts");
...
function makeBrussels(){
  ...
  Promise.all([step0, step1, step2, step3,step4, step5, step6, step7]).then((steps)=>{
    steps.forEach(step=> brusselsSprout.innerHTML += `<li>${step}</li>`)
  })
  .then(()=>{
    brusselsSprout.innerHTML+=`<li>Brussels sprouts are ready!</li>`
  })
  .then(()=>{
