...
let recipeBS = async () => {
  try {
    let overallRecipePromises = [];
    brusselsSprouts.forEach((brusselsSprouts, i) => {
      overallRecipePromises.push(obtainInstruction("brusselsSprouts", i));
    });

    let steps = await Promise.all(overallRecipePromises);
    steps.forEach((step) => {
      document.querySelector(
        "#brusselsSprouts"
