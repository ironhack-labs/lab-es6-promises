...
//     document.querySelector("#mashedPotatoes").innerHTML += `<li>${step}</li>;</li > `;
//   })
// }
...
// ...
function promiseAll() {
  const sprout1 = obtainInstruction("brusselsSprouts", 0);
  const sprout2 = obtainInstruction("brusselsSprouts", 1);
  const sprout3 = obtainInstruction("brusselsSprouts", 2);
  const sprout4 = obtainInstruction("brusselsSprouts", 3);
  const sprout5 = obtainInstruction("brusselsSprouts", 4);
  const sprout6 = obtainInstruction("brusselsSprouts", 5);
  const sprout7 = obtainInstruction("brusselsSprouts", 6);
  const sprout8 = obtainInstruction("brusselsSprouts", 7);
  // const sprout9 = obtainInstruction("brusselsSprouts", 8); 
  document.querySelector("#brusselsSproutsImg").removeAttribute("hidden");

  Promise.all([sprout1, sprout2, sprout3, sprout4, sprout5, sprout6, sprout7, sprout8])
    .then((sprout) => {
      //console.log(sprout);// SPROUT IS AN ARRAY, BEACUSE PROMISE ALL RETURNS AN ARRAY
      sprout.forEach(s => {
        document.querySelector('#brusselsSprouts').innerHTML += `<li>${s}</li>`;
      });
