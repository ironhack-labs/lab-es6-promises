...
obtainInstruction("steak", 0)
  ...
  .then((step5) => {
    document.querySelector("#steak").innerHTNL += `<li>${step5}</li>`;
...
const makeBroccoli = async () => {
  ...
  document.querySelector(
    "#broccoli"
  ).innerHTML += `<li>Brokkoli is ready!</li>`;
...
const makeBrusselsSprouts = async () => {
  ...
  for (let step = 0; step <= 7; step++) {
    promisesArray.push(
      obtainInstruction("brusselsSprouts", step).then((instruction) => {
  ...
  try {
    const stepResults = await Promise.all(promisesArray);
    document.querySelector("#brusselsSprouts").innerHTML = `${stepResults.join("")}`;
  } catch (error) {
    console.error(error);
  }
