...
async function makeBroccoli() {
  try {
    await (obtainInstruction('broccoli', 0)).then( (step1) => {
      document.querySelector("#broccoli").innerHTML += `<li>${step1}</li>`});
    await (obtainInstruction('broccoli', 1)).then( (step2) => {
      document.querySelector("#broccoli").innerHTML += `<li>${step2}</li>`});
    await (obtainInstruction('broccoli', 2)).then( (step3) => {
      document.querySelector("#broccoli").innerHTML += `<li>${step3}</li>`});
    await (obtainInstruction('broccoli', 3)).then( (step4) => {
      document.querySelector("#broccoli").innerHTML += `<li>${step4}</li>`});
    await (obtainInstruction('broccoli', 4)).then( (step5) => {
      document.querySelector("#broccoli").innerHTML += `<li>${step5}</li>`});
    await (obtainInstruction('broccoli', 5)).then( (step6) => {
      document.querySelector("#broccoli").innerHTML += `<li>${step6}</li>`});
    await (obtainInstruction('broccoli', 6)).then( (step7) => {
...
obtainInstruction('brusselsSprouts', 7) ]).then((values) => {values.forEach(element => {
