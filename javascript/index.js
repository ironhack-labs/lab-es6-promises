...
obtainInstruction("steak", 0).then(
  (step0) => {
    document.querySelector("#steak").innerHTML += `<li>${step0}</li>`;

    obtainInstruction("steak", 1).then(
      (step1) => {
        ...
        obtainInstruction("steak", 2).then(
          (step2) => {
            ...
            obtainInstruction("steak", 3).then(
              (step3) => {
                document.querySelector(
                  "#steak"
                ).innerHTML += `<li>${step3}</li>`;

                obtainInstruction("steak", 4).then(
                  (step4) => {
                    document.querySelector(
                      "#steak"
                    ).innerHTML += `<li>${step4}</li>`;

                    obtainInstruction("steak", 5).then(
                      (step5) => {
                        document.querySelector(
                          "#steak"
                            ...
                            obtainInstruction("steak", 7).then(
                              (step7) => {
                                ...
                                document
                                  ...
                                  .removeAttribute("hidden"); //how can I add the image? (this doesn't work)
                                document.querySelector(
                                  "#steak"
...
async function makeBroccoli() {
  try {
    ...
    document.querySelector("#sbroccoliImg").removeAttribute("hidden");
  } catch (error) {
    console.log(error);
  }
}
