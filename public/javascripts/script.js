...
// DOESN'T WORK
/*async function steakCallback(step) {
    if (step < steak.length) {
        step++;
        console.log('step value ', step);
        return addFood(steak[step], '#steak', steakCallback(step));
    } else {
        return false;
    }
}
addFood(steak[0], '#steak', steakCallback(0));
*/
...
async function makeFood(steps, id) {
    for (let i = 0; i < steps; i++) {
        await addFood(brusselSprouts[i], id);
    }
}
makeFood(brusselSprouts.length, '#brusselSprouts').then(() => {
